...
import { fetchAdminPublications, reviewPublication } from '@/lib/api';
    ...
    <div className="mt-8">
      <Card>
        <CardHeader>
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <CardDescription>
                Tìm kiếm theo tiêu đề hoặc tác giả, sau đó thao tác phát hành hoặc gỡ bỏ mềm
                với ghi chú bắt buộc.
              </CardDescription>
            </div>
        ...
        <CardContent>
          <Tabs
            value={status}
            onValueChange={(value) => setStatus(value as PublicationStatus | 'ALL')}
          >
            <TabsList>
                            ...
                            <div className="flex justify-end gap-2">
                              {item.status !== 'PUBLISHED' ? (
                                <Button
                                  size="sm"
                                  onClick={() =>
                                    setDialog({ publicationId: item.id, action: 'publish' })
                                  }
                                >
                                  Phát hành
                                </Button>
                              ...
                              {item.status !== 'SUSPENDED' ? (
                                <Button
                                  ...
                                  size="sm"
                                  onClick={() =>
                                    setDialog({ publicationId: item.id, action: 'suspend' })
                                  }
                                >
                                  Gỡ bỏ
                                </Button>
